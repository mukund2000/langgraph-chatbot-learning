...
# Import necessary libraries for the chatbot with tools and memory
...
# Initialize memory saver to persist conversation state across interactions
memory = MemorySaver()
...
# Compile the graph into an executable application with memory checkpointer
app = graph.compile(checkpointer=memory)

# Configuration for the conversation thread
config = {"configurable": {
    "thread_id": 2  # Unique thread ID for persisting conversation state
}}
...
while True:
    ...
    if(user_input in ["exit", "end"]):
        ...
        # Invoke the app with the user's message and config for memory
        result = app.invoke({
